  ...
  <view class="ronin-details-new">
    ...
    <view class="card" v-if="list.length > 0">
      ...
      <view v-for="(item, index) in list" :key="index">
        <view class="product-item">
          <u-checkbox @change="moneyChange" v-model="item.check"></u-checkbox>
          <view class="product-content product-content-null">
            <view class="product">
              <view class="name">{{ item.productName }}</view>
              <!-- <view class="price">{{ item.memo }} </view> -->
            </view>
...
export default {
  ...
  methods: {
    getList() {
      userAddProjectByAfter({ orderSn: this.id }).then((res) => {
        if (res.code == "0") {
          let data = [];
    ...
    moneyChange() {
      this.$nextTick((res) => {
        ...
        this.allPrice = price + this.details.fund;
      });
    },
    getDetails() {
      userAppointmentInfo({ id: this.id }).then((res) => {
        if (res.code == "0") {
          ...
          this.allPrice = this.details.fund;
          this.getList();
        }
      });
    },
    ...
    onClickSubmit() {
      ...
      userAppointmentUpdate(req).then((res) => {
        if (res.code == "0") {
          uni.navigateBack({
            delta: 1,
          });
