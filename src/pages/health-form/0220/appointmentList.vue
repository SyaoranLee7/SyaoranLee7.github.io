<template>
  <view class="ronin-helth-form">
    ...
    <view class="item" v-for="(item, index) in list" :key="index + '6789okm'">
      <view class="head">
        <view class="head-name"
          >{{ item.id }}
          <!-- <text class="small">女 26岁</text> -->
        </view>
        <view>{{ item.medicalCompanyName }} </view>
      </view>
      <view class="content" @click.stop="onClickDetails(item)">
        ...
        <view class="item-content">
          ...
          <view class="sub-title">{{ item.productNames }}</view>
      ...
      <view class="item-bottom">
        ...
        >
        <u-button
          :disabled="item.status == 'report' ? false : true"
          class="btn"
          size="mini"
          shape="circle"
          type="primary"
    ...
    <u-modal v-model="showTast" :show-cancel-button='true' :async-close="true" @confirm="confirm" content="确定取消该订单吗"></u-modal>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    onClickAppointment(item){
      ...
      let url = `/pages/health-form/appointment?id=${item.id}&companyNo=${item.medicalCompanyNo}`;
      uni.navigateTo({
        url,
      });
    },
